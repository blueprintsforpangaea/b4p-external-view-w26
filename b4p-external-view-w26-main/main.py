...
def _use_sample_data() -> bool:
    return os.environ.get("USE_SAMPLE_DATA", "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
...
def _gspread_client():
    ...
    if not key_path or not Path(key_path).is_file():
        raise HTTPException(
            status_code=500,
            detail="Server missing GOOGLE_APPLICATION_CREDENTIALS or file not found.",
        )
    scopes = [
...
async def submit_request(body: SubmitRequestBody):
    ...
    if _use_sample_data():
        ...
        if cid:
            if cid in _sample_client_request_ids:
                raise HTTPException(
                    status_code=409,
                    detail="Duplicate submission (client_request_id already recorded).",
                )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Google Sheets error while appending: {e}",
        ) from e
...
# ---------------------------------------------------------------------------
# Org Request API — POST /requests  and  PATCH /requests/{id}/status
# ---------------------------------------------------------------------------
...
def _org_requests_worksheet(spreadsheet: gspread.Spreadsheet) -> gspread.Worksheet:
    """Get (or create) the Requests worksheet for org submissions, ensuring headers."""
...
def _review_flag(inventory: list[dict], item_name: str) -> bool:
    """Return True if the matching inventory item has Review = 'Review needed'."""
...
async def update_org_request_status(request_id: str, body: StatusUpdateBody):
    ...
    for i, row in enumerate(all_rows[1:], start=2):  # sheet rows are 1-based; data starts at 2
    ...
    sheet_status_col = status_col + 1  # gspread uses 1-based column index
    try:
        ws.batch_update(
            [
                {
                    "range": gspread.utils.rowcol_to_a1(r, sheet_status_col),
                    "values": [[body.status]],
                }
    ...
    return {"request_id": request_id, "status": body.status, "rows": updated_rows}


# ---------------------------------------------------------------------------
# Availability endpoint — reads live from inventory + Requests sheet
# ---------------------------------------------------------------------------

@app.get("/inventory/availability")
async def inventory_availability():
    """Return per-item availability status cross-referenced with org requests."""
    inventory = get_inventory_records()
            ...
            if len(all_rows) > 1:
                ...
                for r in all_rows[1:]:
                    if any(cell.strip() for cell in r):
                        ...
                        org_requests.append(row_dict)
        except Exception:
            pass  # sheet unavailable → treat as no requests
...
# ---------------------------------------------------------------------------
# Static file serving — active when `make build` has run
# ---------------------------------------------------------------------------

_build_dir = BASE_DIR / "my-dashboard" / "build"
